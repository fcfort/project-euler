...
use Moose;
		...
		while ( my $row = $nextRow->()) {
			...
			for my $boxEl ( @{$row} ) {
				...
				if  ( $boxEl->size != 1 ) {
					...
					$tempSet->insert( $boxEl->members );
				}
			}
			# check if each row contains 1..9
			if ( $tempSet != $allNumberSet ) {
				return 0;
			}
	...
	sub guessFirstLevelOptions {
		...
		while (my $unsolvedEl = $nextUnsolved->() ) {
			...
			$s->solve();
			if ( $s->isSolved() ) {
				print "Sudoku solved in guessFirstLevelOptions()\n";
				return 1;
			} else {
				$s->copy($s_copy);
				next;
